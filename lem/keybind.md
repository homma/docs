...
- `C-x ]` は実質的にバッファの最初に移動するキーとして使用可能です
- `C-x [` は実質的にバッファの最後に移動するキーとして使用可能です
...
### Lem には存在しないキー
- Emacs には存在していて Lem に存在しないキーの一部
