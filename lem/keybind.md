...
| キーバインド | コマンド | 意味 | 補足 |
| --- | --- | --- | --- |
| <kbd>C-x @</kbd> | `pipe-command` | ミニバッファに入力した OS コマンドを実行し、結果を取得する | |
| <kbd>C-x #</kbd> | `filter-buffer` | ミニバッファに入力した OS コマンドに現在のバッファの内容を渡して実行。結果をバッファに書き戻す。 | |

`C-x #` してからミニバッファに `wc -l` と入力すると、現在のバッファの行数がカウントされ、コマンドの出力でバッファが上書きされます。  
macOS では、`C-x #` で `filter-buffer` を実行し、`pbcopy` コマンドを指定するとバッファの内容を OS のクリップボードにコピーできます。
