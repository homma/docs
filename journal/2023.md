...
`nix-user-chroot` を使用することで non-root インストールも可能だが、macOS で動作するかは不明
...
### パッケージ管理
`Homebrew` や `NixPkgs` のパッケージバイナリを流用して自分用のパッケージ管理の仕組みを構築できないか  
ルートディレクトリではなく、ユーザのホームディレクトリ配下でパッケージを管理したい  

まずはそれぞれのパッケージの形式を調査してみる
...
function run(input, parameters) {
  // Automator ignores '\n' but '\n\0'
  const text = `${input[0]}\n\0`;
  const desktop = app.pathTo("desktop").toString();
  const file = `${desktop}/words.txt`;
