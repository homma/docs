...
````lua
luado print(vim.fn.maparg('<C-e>', 'i'))
-- => <End>
luado print(vim.fn.mapcheck('<C-e>', 'i'))
-- => <End>
````
...
````lua
luado print(vim.fn.maparg('<c-p>', 'i') == '')
````

返り値の長さでも判定できる。

````lua
luado print(#vim.fn.maparg('<c-x>', 'i') == 0)
````
...
ただ、一部のサイトでは正しくスケールされないようなので、別の方法を探す必要がありそう  
Firefox や Chrome には専用の zoom 設定が存在している
