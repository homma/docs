...
// original node which holds rotation
let n0 = SCNNode()
n0.simdEulerAngles = vector3(deg2rad(x), deg2rad(y), deg2rad(z))

// get ratation of each axis
let qy = simd_quatf(angle: n0.simdEulerAngles.y, axis: vector3(0, 1, 0))
let qx = simd_quatf(angle: n0.simdEulerAngles.x, axis: vector3(1, 0, 0))

// multiply rotations
var q1 = simd_mul(qy, qy)
q1 = simd_mul(q1, qy)
q1 = simd_mul(q1, qx)

let n1 = SCNNode()
n1.simdOrientation = q1
...
各軸回りの Quaternion を取り出して、回転量を増やしたい軸を複数回掛け合わせる  
...
let angle = QuaternionToEuler(q0)
let qy = QuaternionFromAxisAngle(Vector3(x: 0, y: 1, z: 0), angle.y)
let qx = QuaternionFromAxisAngle(Vector3(x: 1, y: 0, z: 0), angle.x)

// multiply rotations
var q1 = QuaternionMultiply(qy, qy)
q1 = QuaternionMultiply(q1, qy)
q1 = QuaternionMultiply(q1, qx)
