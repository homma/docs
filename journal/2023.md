...
### SDL とイベントループ
SDL の仕組み的には `SDL_PollEvent` でイベントを待ち受ける
`SDL_PollEvent` は SDL のイベントのみを待ち受ける

ソケットや標準入力など、SDK 外のイベントを待つことはできない
...
SDL 以外のイベントを待ちたいなら、マルチスレッドかマルチプロセスにしてキューイングするなどの対応が必要そう
