...
ターゲットは myapp と curses  
myapp が C の関数を呼び出すプログラム  
curses が既存の C ライブラリ  
それぞれ Sources/<ターゲット名>/ のディレクトリを作成する
...
let package = Package(
  ...
  targets: [
    .executableTarget(name: "myapp",
                      dependencies: ["curses"],
                      linkerSettings: [ .linkedLibrary("curses")]),
...
この場合はファイル名w module.map ではなく module.modulemap にする必要がある
